...
BEGIN
	...
	IF conta_existente > 0 THEN 
        ...
        SET MESSAGE_TEXT = "O clinete já possui uma conta nesta agência";
	END IF;
...
BEGIN
IF NEW.tipo_transacao IN ('depósito', 'estorno') THEN
        UPDATE conta SET saldo = saldo + NEW.valor WHERE conta_num_conta = NEW.conta_num_conta;
    ELSEIF NEW.tipo_transacao IN ('saque', 'pagamento', 'transferência', 'PIX') THEN
        IF (SELECT saldo FROM conta WHERE conta_num_conta = NEW.conta_num_conta) - NEW.valor < 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Saldo insuficiente para realizar a transação.';
        ELSE
            UPDATE contas SET saldo = saldo - NEW.valor WHERE conta_num_conta = NEW.conta_num_conta;
        END IF;
    END IF;
END$$
